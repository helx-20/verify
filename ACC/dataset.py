...
class critical_data(Dataset):
    def __init__(self, range, transform = None, target_transform = None):
        ...
        self.all_data = np.load("/mnt/mnt1/linxuan/nnv/ACC/dataset/critical_data4.npy", allow_pickle=True).item()
...
def get_dataset(batch_size):
    ...
    train_dataset = critical_data(order[:180000])
    train_loader = DataLoader(dataset=train_dataset,
                              batch_size=batch_size,
                              shuffle=True,
                              num_workers=0)
     
    test_dataset = critical_data(order[180000:200000])
    test_loader = DataLoader(dataset=test_dataset,
                              batch_size=batch_size,
                              shuffle=False,
                              num_workers=0)
    
    val_dataset = critical_data(order[200000:])
