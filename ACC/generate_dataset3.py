...
def load_data(idx):
    with open("/mnt/mnt1/linxuan/nnv/ACC/train_data3_critical_json/{}.json".format(idx), 'r') as file:
        data = json.load(file)
    return data
...
def select_range(idx):
    ...
    if isinstance(data_tmp, list):
        for i in range(5):
            ...
            data.append(np.array(data_tmp[0]['Uin'])[i,0])
            data.append(np.array(data_tmp[0]['Uin'])[i,i])
        ...
        for i in range(5):
            ...
            data.append(np.array(data_tmp['Uin'])[i,0])
            data.append(np.array(data_tmp['Uin'])[i,i])
...
np.save("/mnt/mnt1/linxuan/nnv/ACC/dataset/critical_data5.npy", {'data':data, 'label':label})
