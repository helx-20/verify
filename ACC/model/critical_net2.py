...
from .bert2 import Embeddings, Block, Config
import math
...
def define_critical_net2(model, input_dim, output_dim, m_tokens_in, m_tokens_out):
    ...
    if model == 'transformer':
        ...
        bert_cfg.out_tokens = m_tokens_out
        Backbone = Out_net(input_dim=M.output_dim, output_dim=output_dim, m_tokens=m_tokens_in, cfg=bert_cfg)
    else:
...
class Transformer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, m_tokens, cfg=Config()):
        ...
        self.blocks = nn.ModuleList([Block(cfg,'cross')]+[Block(cfg,'self') for _ in range(cfg.n_layers-1)])
...
class Out_net(nn.Module):
    def __init__(self, input_dim, output_dim, m_tokens, cfg=Config()):
        ...
        self.m_tokens = m_tokens
        self.mask_net = Transformer(input_dim=input_dim, output_dim=1, m_tokens=m_tokens, cfg=cfg)
        self.output_set_net = Transformer(input_dim=input_dim, output_dim=output_dim, m_tokens=m_tokens, cfg=cfg)
        self.mask_thr = 0.5
        self.judge_critical = Critical.apply 

    def forward(self, x):
        #print(self.mask_net(x))
        mask = self.mask_net(x)
        #mask = self.judge_critical(self.mask_net(x) - self.mask_thr)
        output_set = self.output_set_net(x)
