...
class PositionalMapping(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.trans(0,x_scale,h,PI)
        h = self.trans(1,x_scale,h,PI)
        h = self.trans(2,x_scale,h,PI)
        h = self.trans(3,x_scale,h,PI)

        return torch.cat(h, dim=-1)

class Critical_Net(nn.Module):
    ...
    def forward(self, V_C_d):
        print('here')
        start_time = time.time()
        V = V_C_d[:,:,:self.input_dim_V]
        C_d = V_C_d[:,:,self.input_dim_V:]
        batchsize = V.shape[0]
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.in_net(x)
        h = self.blocks[0](h, None)
        h = self.blocks[1](h, None)
        h = self.blocks[2](h, None)
        h = self.blocks[3](h, None)
        y = self.out_net(h)
        return y