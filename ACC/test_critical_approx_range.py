...
data_path = "/mnt/mnt1/linxuan/nnv/ACC/new_train_data3_critical_json"
label = []
for i in order[500000:545000]:
    with open(os.path.join(data_path,'Rc_convex_{}.json'.format(i+1)), 'r') as file:
        data = json.load(file)
        label.append(data)
label = torch.tensor(label, dtype=torch.float32)

data_path = "/mnt/mnt1/linxuan/nnv/ACC/new_train_data3_critical_approx_json"
label_approx = []
for i in order[500000:545000]:
    with open(os.path.join(data_path,'Rc_convex_{}.json'.format(i+1)), 'r') as file:
        data = json.load(file)
