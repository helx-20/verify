...
model_mask = define_critical_mask_net(model='transformer', input_dim=2, output_dim=2, m_tokens_in=5, m_tokens_out=48, transformer_out_feature_dim=1024)
model_mask.load_state_dict(torch.load("saved_model/critical_mask_net.pth").state_dict())
model_mask = model_mask.to(device)
model_set = define_critical_set_net(model='transformer', input_dim=2, output_dim=2, m_tokens_in=5, m_tokens_out=48, transformer_out_feature_dim=1024)
model_set.load_state_dict(torch.load("saved_model/critical_set_net.pth").state_dict())
model_set = model_set.to(device)
...
def test(device, test_loader, criterion, epoch):
    ...
    with torch.no_grad():
        for data, target in test_loader: 
            ...
            #mask = (target[:,:,0] != 0).float().view(target.shape[0], -1, 1)
            mask = model_mask(data)
            out_set = model_set(data)
            output = mask * out_set
