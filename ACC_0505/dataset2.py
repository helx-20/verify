...
class critical_data(Dataset):
    def __init__(self, idx_range, transform = None, target_transform = None):
        ...
        self.data_path = "/mnt/mnt1/linxuan/nnv/ACC/new_train_data2_uncritical_json"
...
def get_dataset(batch_size):
    order = np.array(range(1730241))
    random.shuffle(order)

    train_dataset = critical_data(order[:1500000])
    train_loader = DataLoader(dataset=train_dataset,
                              batch_size=batch_size,
                              shuffle=True,
                              num_workers=0)
     
    test_dataset = critical_data(order[1500000:1600000])
    test_loader = DataLoader(dataset=test_dataset,
                              batch_size=batch_size,
                              shuffle=False,
                              num_workers=0)
    
    val_dataset = critical_data(order[1600000:])
