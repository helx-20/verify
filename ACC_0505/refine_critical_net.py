...
model.load_state_dict(torch.load("saved_model/critical_net4_state_dict.pth"))
...
def train(model, device, train_loader, optimizer, scheduler, criterion, epoch, criterion2 = nn.L1Loss(reduction='sum')):
    ...
    for batch_id, (data_V, data_C_d, target) in enumerate(train_loader): 
        ...
        #critical = critical.view(-1,1)
        loss = criterion(output, target) + 1e-1 * criticality #+ 1e-3 * regularization #+ 1e-1 * criticality * criterion2(output[critical_place], target[critical_place])
        print(criticality)
        print(loss.item())
        loss.backward()
        optimizer.step()
...
def test(model, device, test_loader, criterion, epoch, criterion2 = nn.L1Loss(reduction='sum')):
    ...
    with torch.no_grad():
        for data_V, data_C_d, target in test_loader: 
            ...
            critical_place = torch.where(critical > 0)
            #loss = criterion(output, target) + 1e-1*criterion2(output[critical_place], target[critical_place])
            loss = criterion(output, target) + 1e-1 * criticality #+ 1e-3 * regularization
            test_loss += loss.item()
...
def main(model):
    ...
    for e in range(Epoch):
        ...
        if loss < min_loss:
            min_loss = loss
            best_model = model
            torch.save(model.state_dict(),'saved_model/critical_net3_state_dict.pth')
            print("save")
