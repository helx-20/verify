...
#model.load_state_dict(torch.load("saved_model/critical_net.pth").state_dict())
...
def main(model):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[150,600], gamma=0.2)
    criterion = nn.L1Loss()
    ...
    for e in range(Epoch):
        ...
        if loss < min_loss:
            min_loss = loss
            best_model = model
            torch.save(model,'saved_model/critical_net2.pth')
            print("save")
